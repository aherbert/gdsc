 ...
 * (i.e. where the gradient is steepest). Optionally allows: the edge lines to be pruned to leave
 * only closed loops; and filling of loops to create a mask. <p> Requires the ImageScience library
 * that supports the Laplacian plugin of FeatureJ. The imagescience.jar must be installed in the
...
public class EdgeMask implements ExtendedPlugInFilter, DialogListener {
  ...
  private class MaskCreator implements Runnable {
    ...
    public MaskCreator(ImageProcessor ip, ImageStack outputStack, int slice, EdgeMask mask) {
      this.ip = ip;
