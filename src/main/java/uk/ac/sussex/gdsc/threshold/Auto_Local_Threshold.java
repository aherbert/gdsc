...
/**
 * AutoLocalThreshold segmentation plugin. <p> Adapted from the ImageJ plugin by G. Landini at bham.
 * ac. uk.
 */
public class Auto_Local_Threshold implements PlugIn {
  ...
  public void run(String arg) {
    ...
    final String[] methods =
        {"Try all", "Bernsen", "Mean", "Median", "MidGrey", "Niblack", "Sauvola"};
    ...
    if (myMethod.equals("Try all")) {
      ...
      if (stackSize > 1 && doIstack) {
        if (stackSize > 25) {
          final YesNoCancelDialog d = new YesNoCancelDialog(IJ.getInstance(),
              "Auto Local Threshold", "You might run out of memory.\n \nDisplay " + stackSize
      ...
      return;
    } else if (stackSize > 1 && doIstack) { // whole stack
                                            // if (doIstackHistogram) {// one global histogram
                                            // Object[] result = exec(imp, myMethod, noWhite,
                                            // noBlack, doIwhite, doIset, doIlog, doIstackHistogram
                                            // );
                                            // }
                                            // else{ // slice by slice
   ...
   *
   * @param imp the image
   * @param myMethod the threshold method
   * @param radius the radius
   * @param par1 the parameter 1
   * @param par2 the parameter 2
  ...
  private static void MidGrey(ImagePlus imp, int radius, double par1, boolean doIwhite) {
    ...
    for (int i = 0; i < pixels.length; i++) {
      pixels[i] =
          ((pixels[i] & 0xff) > ((max[i] & 0xff) + (min[i] & 0xff)) / 2 + c_value) ? object : backg;
    }
    // imp.updateAndDraw();
    return;
  }
  ...
  private static void Niblack(ImagePlus imp, int radius, double par1, boolean doIwhite) {
    ...
    for (int i = 0; i < pixels.length; i++) {
      pixels[i] =
          ((pixels[i] & 0xff) > (int) (mean[i] + k_value * Math.sqrt(var[i]))) ? object : backg;
    }
    // imp.updateAndDraw();
    return;
  }

  private static void Sauvola(ImagePlus imp, int radius, double par1, double par2,
      boolean doIwhite) {
    ...
    for (int i = 0; i < pixels.length; i++) {
      pixels[i] = ((pixels[i]
          & 0xff) > (int) (mean[i] * (1.0 + k_value * ((Math.sqrt(var[i]) / r_value) - 1.0))))
              ? object
              : backg;
    }
    // imp.updateAndDraw();
