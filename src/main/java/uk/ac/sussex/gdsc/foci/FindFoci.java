...
public class FindFoci implements PlugIn, MouseListener, FindFociProcessor
{
			...
			int centreMethod, double centreParameter, double fractionParameter)
	{
		...
		final FindFociResults ffResult = findMaxima(ffp, imp, mask, backgroundMethod, backgroundParameter,
		...
		if (maximaImp != null)
		{
			if ((outputType & OUTPUT_MASK) != 0)
			{
				...
				{
					final int maxValue = ((outputType & OUTPUT_MASK_THRESHOLD) != 0) ? 4 : resultsArray.size() + 1;
			...
			String batchPrefix)
	{
				...
				new FileOutputStream(resultsDirectory + File.separatorChar + expId + ".xls"), "UTF-8"))
		{
			...
			final ArrayList<String> batchResults = (allOut == null) ? null : new ArrayList<>(resultsArray.size());
			for (int i = 0; i < resultsArray.size(); i++)
			{
				final FindFociResult result = resultsArray.get(i);
				xpoints[i] = result.x;
	 ...
	 * (non-Javadoc)
	 *
	 * @see uk.ac.sussex.gdsc.foci.FindFociProcessor#findMaxima(ij.ImagePlus, ij.ImagePlus, int, double, java.lang.String, int,
	 ...
	 * (non-Javadoc)
	 *
	 * @see uk.ac.sussex.gdsc.foci.FindFociProcessor#findMaximaInit(ij.ImagePlus, ij.ImagePlus, ij.ImagePlus, int, java.lang.String,
	 * int)
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 *
	 * @see uk.ac.sussex.gdsc.foci.FindFociProcessor#clone(uk.ac.sussex.gdsc.foci.FindFociInitResults, uk.ac.sussex.gdsc.foci.FindFociInitResults)
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 *
	 * @see uk.ac.sussex.gdsc.foci.FindFociProcessor#findMaximaSearch(uk.ac.sussex.gdsc.foci.FindFociInitResults, int, double, int, double)
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 *
	 * @see uk.ac.sussex.gdsc.foci.FindFociProcessor#findMaximaResults(uk.ac.sussex.gdsc.foci.FindFociInitResults, uk.ac.sussex.gdsc.foci.FindFociMergeResults,
	 * int, int, int, double)
	 */
	@Override
	...
	{
		lastResultsArray = null;
		final FindFociResults result = ffpStaged.findMaximaResults(initResults, mergeResults, maxPeaks, sortIndex,
	 ...
	 * uk.ac.sussex.gdsc.foci.FindFociMergeResults, uk.ac.sussex.gdsc.foci.FindFociResults, int, java.lang.String, java.lang.String, double)
	 */
	@Override
	...
	private class Worker implements Runnable
	{
		...
		private void run(String filename, int batchId)
		{
			...
			final MemoryLogger logger = new MemoryLogger();
	...
	{
		for (final String message : logger.getMessages())
			IJ.log(message);
	}
