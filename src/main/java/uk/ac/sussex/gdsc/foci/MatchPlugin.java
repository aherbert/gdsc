...
public class MatchPlugin implements PlugIn
{
	...
	private static FindFociMemoryResults getFindFociMemoryResults(String resultsName) throws IllegalStateException
	{
			...
			throws IllegalStateException
	{
		...
		if (unitType == 1)
		{
			...
			final uk.ac.sussex.gdsc.core.match.BasePoint[] points = new uk.ac.sussex.gdsc.core.match.BasePoint[results.size()];
			int i = 0;
			for (final FindFociResult result : results)
				points[i++] = new uk.ac.sussex.gdsc.core.match.BasePoint(convert(result.x, scaleX), convert(result.y, scaleY),
			...
			List<Coordinate> falsePositives, List<Coordinate> falseNegatives)
	{
		...
		final Plot plot = new Plot(title, imp1.getTitle(), imp2.getTitle(), (float[]) null, (float[]) null);
			...
			List<Coordinate> falsePositives, List<Coordinate> falseNegatives)
	{
		...
		// Show a result table
		final String header = "Image 1\tN\t% <Q1\t% Q1\t% Q2\t% Q3\t% Q4\t% >Q4\tImage2\tN\t% <Q1\t% Q1\t% Q2\t% Q3\t% Q4\t% >Q4";
		if (!java.awt.GraphicsEnvironment.isHeadless())
		{
	...
	private static void addUnmatchedResult(String title1, String title2, int[] actualCount, int[] predictedCount)
	{
		final StringBuilder sb = new StringBuilder();
