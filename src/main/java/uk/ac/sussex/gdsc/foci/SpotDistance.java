...
/**
 * Output the distances between spots within a mask region. <P> For each mask region a Difference of
 * Gaussians is performed to enhance spot features. The spots are then identified and the distances
 * between spots are calculated. <p> A mask can be defined using a freehand ROI or provided as an
 * image. All pixels with the same non-zero value are used to define a mask region. The image can
 * thus be used to define multiple regions using pixel values 1,2,3, etc.
 */
public class SpotDistance implements PlugIn {
  ...
  private ImagePlus checkMask(ImagePlus imp, String maskImage) {
    ...
    if (imp.getNSlices() > 1 && maskImp.getNSlices() != 1
        && maskImp.getNSlices() != imp.getNSlices()) {
      IJ.showMessage("Error", "Mask region has incorrect slice dimension");
      return null;
    }
    if (imp.getNChannels() > 1 && maskImp.getNChannels() != 1
        && maskImp.getNChannels() != imp.getNChannels()) {
  ...
  private void createResultsWindow() {
    ...
    if (resultsWindow == null || !resultsWindow.isShowing()
        || !sameHeader(resultsWindow, resultsHeader)) {
    ...
    if (summaryWindow == null || !summaryWindow.isShowing()
        || !sameHeader(summaryWindow, summaryHeader)) {
  ...
  private void exec(int frame, int channel, ImageStack s1, ImageStack s1b, ImageStack s2) {
    ...
    for (final int region : regions) {
      ...
      final ArrayList<DistanceResult> resultsArray =
          analyseResults(prevResultsArray, ff, croppedImp, ffResult, frame, channel, overlay);
   ...
   * values). <p> Only scale the image if the input image stack is a FloatProcessor. The stack will
   * be converted to a ShortProcessor.
   *
   * @param s1 The image
   * @return The scale
   */
   ...
   * @return the results
   */
  private ArrayList<DistanceResult> analyseResults(ArrayList<DistanceResult> prev, FindFoci ff,
    ...
    if (prev != null && !prev.isEmpty() && !newResultsArray.isEmpty()) {
      ...
      final double d = Math.sqrt(croppedImp.getWidth() * croppedImp.getWidth()
