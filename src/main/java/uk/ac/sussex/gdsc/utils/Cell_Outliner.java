...
/**
 * Outlines a circular cell using the optimal path through a membrane scoring map. <p> The centre of
 * the cell must be chosen and an approximate cell radius. Only pixels within a range of the cell
 * radius are processed. The score map is created using 2D convolutions of a curve through a NxN
 * square. The curve is constructed as a difference-of-Gaussians approximation to the Laplacian
 * allowing dark/light edge detection. The curve arc uses the cell radius and is rotated to produce
 * membrane edge projections around the centre point. <p> The maximum intensity membrane projection
 * is weighted using the current elliptical fit of the cell (initialised as a circle using the cell
 * radius). A polygon outline is constructed from the projection using the highest value in each 10
 * degree segment around the selected centre. An elliptical shape consisting of two ellipses
 * back-to-back (allowing egg-like shapes) is then fitted to the polygon outline and the process
 * iterated.
 */
