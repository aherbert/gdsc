 ...
 * (CDA).
 *
 * Colocalisation of two channels is measured using Mander's coefficients (M1 &amp; M2) and
 ...
 *
 * Supports stacks. Only a specific channel and time frame can be used and all input images must
 * have the same number of z-sections for the chosen channel/time frame. <p> This class is based on
 * the original CDA_Plugin developed by Maria Osorio-Reich: <a href=
...
public class CDA_Plugin extends PlugInFrame
  ...
  private static String[] ROI_OPTIONS =
  ...
  private final String OPT_SHOW_MERGED_CHANNEL_DISPLACEMENT_RGB =
      "CDA.showMergedChannelDisplacement";
  private final String OPT_SHOW_MERGED_SEGMENTED_DISPLACEMENT_RGB =
  ...
  public synchronized void actionPerformed(ActionEvent e) {
    ...
    if ((Button) actioner == helpButton) {
      final String macro =
          "run('URL...', 'url=" + uk.ac.sussex.gdsc.help.URL.COLOCALISATION + "');";
  ...
  /**
   * Generate random shift indices between the minimum and maximum radius. <p> The indices are
   * packed as unsigned bytes in the first 2 bytes of the integer. The signs are packed in the next
   * 2 bits:
   *
   * <pre>
   * <code>
  ...
  private static boolean checkDimensions(int w, int h, int s, ImageStack stack) {
    if (stack != null
        && (w != stack.getWidth() || h != stack.getHeight() || s != stack.getSize())) {
  ...
  private void reportResults(ImagePlus imp1, ImagePlus imp2, ImageStack imageStack1,
      ...
      double M2, double R, List<CalculationResult> results) {
    ...
    if (showRPlotWindow) {
      plotR = createPlot(distances, rValues, Color.blue, Color.green, plotRTitle, plotXLabel,
    ...
    ImageStack mergedChannelIP = null, mergedSegmentedRGBIP = null,
        mergedChannelDisplacementIP = null, mergedSegmentedDisplacementIP = null;
    ...
    rStatistics = refreshDisplayStatistics(rStatistics, showRStatistics, rValuesFP,
        rValuesForRandom, rHistogramTitle, Color.blue, "R ", R, OPT_LOCATION_STATS_R);
